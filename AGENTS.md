...
- `get_chapter_pages` sends `split=yes`, so every two-page spread arrives as a `MangaPage.type` RIGHT (2) half followed by its LEFT (1) half; DOUBLE (3) only appears with `split=no`. `buildPageGroups` in `src/lib/readerLogic.ts` keeps those two halves in one frame, and the offset of a chapter's first spread decides whether its cover binds solo (odd offset, or no spread) or pairs start on page 1 (even offset). Changing `split` breaks both. The reader has two layouts, `single` and `double`; a stored `double-cover` from the former three-layout cycle is read as `double`.
