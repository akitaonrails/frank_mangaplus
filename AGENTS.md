...
- Reader image retries bust the cache with a `?mpretry=N` **query** param (`retryImageSrc`), never a fragment — fragment-only src changes are excluded from the resource identity and never refetch. The `mpimg://` handler strips `mpretry` (`strip_retry_param` in `lib.rs`) before forwarding so the CDN's signed query is untouched, and the reader's `{#each}` is keyed on `url|attempts` so a retry recreates the `<img>` element (WebKit remembers a failed load on the element itself).
- `get_chapter_pages` sends `split=yes`, so a two-page spread usually arrives as a `MangaPage.type` RIGHT (2) half followed by its LEFT (1) half; DOUBLE (3) only appears with `split=no`. `buildPageGroups` in `src/lib/readerLogic.ts` keeps those two halves in one frame, and the offset of a chapter's first spread decides whether its cover binds solo (odd offset, or no spread) or pairs start on page 1 (even offset). Changing `split` breaks both. The reader has two layouts, `single` and `double`; a stored `double-cover` from the former three-layout cycle is read as `double`. Caveat (not universal): some titles deliver real spreads UNMARKED as consecutive type-0 singles — One Piece ch1-3 return every page type-0, so detection never fires and the chapter falls to the cover-solo default. That default matches print for those chapters only by parity luck; see `reader/docs/spread-pairing-audit.md`.
